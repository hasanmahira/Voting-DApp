...
describe("VotingCore E2E Tests", function () {
    ...
    beforeEach(async function () {
        ...
        // Deployer of TokenRegistry is admin. It should use addToken from the full TokenRegistry.
        // TokenType.ERC20 is 0. Governance token (MGT) will have weight 1.
        const TokenType = { ERC20: 0, ERC721: 1, ERC1155: 2 }; 
        await tokenRegistry.connect(deployer).addToken(governanceToken.address, TokenType.ERC20, 1);
    ...
    describe("Proposal Creation", function () {
        ...
        it("Should allow an account with sufficient RAW voting power (>= threshold) to create a proposal", async function () {
            // proposalCreator has 500 MGT (raw power = 500 * 1 = 500)
            // PROPOSAL_THRESHOLD_VOTES is 100 (raw)
            expect(await tokenRegistry.getRawVotingPower(proposalCreator.address)).to.equal(ethers.utils.parseUnits("500", 18));
        ...
        it("Should revert if proposer has insufficient RAW voting power (< threshold)", async function () {
            // otherUser has 0 MGT. Threshold is 100 raw.
        ...
        it("Should allow proposal if RAW voting power is equal to threshold", async function () {
            // Give otherUser exactly PROPOSAL_THRESHOLD_VOTES (100 MGT raw)
            // First, ensure otherUser has 0, then transfer.
            const currentOtherUserBalance = await governanceToken.balanceOf(otherUser.address);
            await governanceToken.connect(otherUser).transfer(deployer.address, currentOtherUserBalance); // Send back to deployer
            expect(await governanceToken.balanceOf(otherUser.address)).to.equal(0);

            await governanceToken.connect(deployer).transfer(otherUser.address, PROPOSAL_THRESHOLD_VOTES);
            expect(await tokenRegistry.getRawVotingPower(otherUser.address)).to.equal(PROPOSAL_THRESHOLD_VOTES);
        ...
        it("Should revert if proposer has RAW voting power just below threshold", async function () {
            // Give otherUser slightly less than threshold (99 MGT raw)
            const justBelowThreshold = PROPOSAL_THRESHOLD_VOTES.sub(ethers.utils.parseUnits("1", 18));
            const currentOtherUserBalance = await governanceToken.balanceOf(otherUser.address);
            await governanceToken.connect(otherUser).transfer(deployer.address, currentOtherUserBalance); // Send back to deployer
            expect(await governanceToken.balanceOf(otherUser.address)).to.equal(0);
            
            await governanceToken.connect(deployer).transfer(otherUser.address, justBelowThreshold);
            expect(await tokenRegistry.getRawVotingPower(otherUser.address)).to.equal(justBelowThreshold);
    ...
    describe("Voting Process", function () {
        ...
        it("Should use quadratic voting power when casting votes", async function () {
            // Raw MGT balances (weight 1 from TokenRegistry setup):
            // voter1: 300 MGT -> raw 300
            // voter2: 300 MGT -> raw 300
            // voter3: 200 MGT -> raw 200
            const qv = (raw) => Math.floor(Math.sqrt(raw)); // Helper for this test

            const voter1RawPower = await tokenRegistry.getRawVotingPower(voter1.address);
            const voter2RawPower = await tokenRegistry.getRawVotingPower(voter2.address);
            const voter3RawPower = await tokenRegistry.getRawVotingPower(voter3.address);

            expect(voter1RawPower).to.equal(ethers.utils.parseUnits("300",18));
            expect(voter2RawPower).to.equal(ethers.utils.parseUnits("300",18));
            expect(voter3RawPower).to.equal(ethers.utils.parseUnits("200",18));

            const voter1QV = qv(300); // sqrt(300) = 17
            const voter2QV = qv(300); // sqrt(300) = 17
            const voter3QV = qv(200); // sqrt(200) = 14
            
            expect(await tokenRegistry.getVotingPower(voter1.address)).to.equal(voter1QV);
            expect(await tokenRegistry.getVotingPower(voter2.address)).to.equal(voter2QV);
            expect(await tokenRegistry.getVotingPower(voter3.address)).to.equal(voter3QV);

            // Voter 1 votes FOR
            await expect(votingCore.connect(voter1).castVote(proposalId, 1)) // 1 = For
                .to.emit(votingCore, "VoteCast")
                .withArgs(voter1.address, proposalId, 1, voter1QV);

            let proposalDetails = await votingCore.getProposalDetails(proposalId);
            expect(proposalDetails.forVotes).to.equal(voter1QV);
            ...
            expect(receipt.votes).to.equal(voter1QV);
            ...
            await expect(votingCore.connect(voter2).castVoteWithReason(proposalId, 0, reason)) // 0 = Against
                .to.emit(votingCore, "VoteCast")
                .withArgs(voter2.address, proposalId, 0, voter2QV);

            proposalDetails = await votingCore.getProposalDetails(proposalId);
            expect(proposalDetails.againstVotes).to.equal(voter2QV);
            receipt = await votingCore.getReceipt(proposalId, voter2.address);
            expect(receipt.hasVoted).to.be.true;
            expect(receipt.support).to.equal(0);
            expect(receipt.votes).to.equal(voter2QV);
            
            // Voter 3 votes ABSTAIN
            await expect(votingCore.connect(voter3).castVote(proposalId, 2)) // 2 = Abstain
                .to.emit(votingCore, "VoteCast")
                .withArgs(voter3.address, proposalId, 2, voter3QV);

            proposalDetails = await votingCore.getProposalDetails(proposalId);
            expect(proposalDetails.abstainVotes).to.equal(voter3QV);
            expect(proposalDetails.forVotes).to.equal(voter1QV); // Ensure forVotes unchanged
            expect(proposalDetails.againstVotes).to.equal(voter2QV); // Ensure againstVotes unchanged
            receipt = await votingCore.getReceipt(proposalId, voter3.address);
            expect(receipt.hasVoted).to.be.true;
            expect(receipt.support).to.equal(2);
            expect(receipt.votes).to.equal(voter3QV);
        });
